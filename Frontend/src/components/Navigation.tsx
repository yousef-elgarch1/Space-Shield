...
import { Satellite, Radar, AlertTriangle, BarChart3, Settings, Menu, X, Globe } from 'lucide-react';
...
const Navigation: React.FC<NavigationProps> = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <>
      ...
      {/* Language Selector Button - Always visible in top right corner */}
      <button 
        className="fixed top-4 right-16 z-50 p-2 bg-space-dark-blue bg-opacity-70 hover:bg-opacity-100 text-space-satellite-blue rounded-md transition-all duration-200 shadow-lg"
      ...
      {showLanguageSelector && (
        <div className="fixed top-14 right-16 z-50 bg-space-dark-blue bg-opacity-90 backdrop-blur-md border border-space-blue-900/30 rounded-md shadow-lg p-3 animate-fade-in">
          <LanguageSelector />
        </div>
      )}
      ...
      `}>
        ...
        <div className="hidden md:block md:absolute md:bottom-0 md:left-0 md:right-0 md:p-4">
           ...
           <div className="mb-4 p-3 rounded-md bg-space-satellite-blue bg-opacity-10 border border-space-satellite-blue border-opacity-20">
            <div className="flex items-center mb-2">
              <Globe size={16} className="mr-2 text-space-satellite-blue" />
              <span className="text-sm text-space-light-gray">{t('common.language')}</span>
            </div>
          ...
          <div className="p-3 rounded-md bg-space-satellite-blue bg-opacity-10 border border-space-satellite-blue border-opacity-20">
            ...
            <div className="text-xs text-muted-foreground mt-1">
              {t('common.lastUpdated')}: 2025-04-09 14:32
            </div>
          </div>
        </div>
      </div>
    ...
    {(isMenuOpen || showLanguageSelector) && (
        <div 
