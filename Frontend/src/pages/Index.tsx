...
const Index = () => {
  ...
  const handleSelectDebris = (debrisId: string, debris: any) => {
    ...
    toast.info(
      <div>
        <h3 className="font-bold text-space-satellite-blue">{debris.name}</h3>
        <p className="text-sm mt-1">{debris.info.substring(0, 80)}...</p>
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-space-deep-black">
      ...
      <div className={`fixed inset-0 z-0 ${fullscreenEarth ? 'z-30' : ''}`}>
      ...
      <div className={`fixed top-4 right-4 z-50`}>
        <button 
          ...
          title={fullscreenEarth ? "Exit Fullscreen" : "Enter Fullscreen"}
        >
          {fullscreenEarth ? <Minimize2 size={18} /> : <Maximize2 size={18} />}
        </button>
      </div>
      
      <div className={`flex-1 p-4 md:p-6 md:ml-64 relative ${fullscreenEarth ? 'z-0 opacity-0 pointer-events-none' : 'z-10 opacity-100'} transition-opacity duration-300`}>
        <div className="mb-6">
        ...
        {activeTab === 'dashboard' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2">
              <div className="p-6 bg-space-dark-blue bg-opacity-30 backdrop-blur-sm rounded-lg border border-space-blue-900/30">
                <h2 className="text-lg font-semibold text-space-light-gray mb-4">Earth Orbital View</h2>
                <p className="text-muted-foreground">
                  Viewing full 3D Earth visualization in background. Use the fullscreen toggle to focus on the Earth view.
                </p>
              </div>
            </div>
